...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    //conn.sendHydrated(m.chat, text.trim(), conn.getName(conn.user.jid), await genProfile(conn, m), 'https://youtube.com/channel/UC0hs6KogavQ', 'YouTube', null, null, [['Speedtest', _p + 'ping'], ['Owner', _p + 'owner']], m)
   ...
   conn.sendButton(m.chat, `*${wish()}, ${name} 👋*`, text.trim(), await genProfile(conn, m), [['Speedtest', _p + 'ping'], ['Owner', _p + 'owner']], false, { quoted: fkon, contextInfo: { externalAdReply: { showAdAttribution: true,
    ...
    mediaType: "VIDEO",
    description: "tukang sadbuy verdi", 
    title: 'kita jomblo bro :(',
    body: wm,
    thumbnail: thumb,
    sourceUrl: sgc
}
